...
//	parser.setDebugModeOn();
	...
	logger.info("Создали таблицу индентификаторов.");	
	treeSemantic.accept(new IdTableFiller(new IdParsigStrategy(), idTable, pythonDir));	//Here we can check nodes
	
	logger.info("Заполнили таблицу идентификаторов.");	
	...
	logger.info("GenerateEQ: Before tableFiller");
	tree.accept(new IdTableFiller(new IdParsigStrategy(), table, pythonDir));
	logger.info("GenerateEQ: After tableFiller");
